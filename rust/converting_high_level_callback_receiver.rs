...
use crate::{
    ...
    converting_callback_receiver::{
        CallbackRecvError, CallbackRecvTimeoutError, CallbackTryRecvError,
    ...
    > ConvertingHighLevelCallbackReceiver<PayloadT, ResultT, T>
{
    ...
    fn recv_stream_chunk(&mut self, chunk: &T) -> Option<(Vec<PayloadT>, ResultT)> {
        ...
        if self.currently_receiving_stream
            && (chunk.ll_message_chunk_offset() != self.chunk_offset
                || chunk.ll_message_length() != self.message_length)
        {
        ...
        let read_length = std::cmp::min(
            chunk.ll_message_chunk_data().len(),
            self.message_length - self.chunk_offset,
        );
        self.buf[self.chunk_offset..self.chunk_offset + read_length]
            .copy_from_slice(&chunk.ll_message_chunk_data()[0..read_length]);
...
impl<
        ...
        ResultT,
        T: FromByteSlice + LowLevelRead<PayloadT, ResultT>,
    > Iterator for ConvertingHighLevelCallbackReceiver<PayloadT, ResultT, T>
{
